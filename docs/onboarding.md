...
Один проход сверху вниз. **Зелёное** считает Python (цифры, гипотезы, графики, отчёт). **Оранжевое** — единственный вызов LLM: короткий комментарий к уже посчитанным фактам. Вкладки справа загораются по SSE сразу, как в `results` появляется ключ, а не в конце.

```mermaid
flowchart TB
    classDef ui fill:#E3F2FD,stroke:#1565C0,color:#0D47A1
    classDef py fill:#E8F5E9,stroke:#2E7D32,color:#1B5E20
    classDef llm fill:#FFF8E1,stroke:#EF6C00,color:#E65100
    classDef job fill:#F3E5F5,stroke:#6A1B9A,color:#4A148C
    classDef out fill:#ECEFF1,stroke:#455A64,color:#263238

    U1["Пользователь загружает CSV / Excel<br/>до 10 файлов. Лист Excel = отдельная таблица"]:::ui
    U2["Число графиков 10 / 15 / 20 / 30<br/>и модель аналитика из настроек"]:::ui
    U3["Кнопка «Запустить анализ»"]:::ui
    U1 --> U2 --> U3

    U3 --> A1["POST /api/analyze"]:::job
    A1 --> A2["Проверка: .csv / .xlsx, лимит файлов,<br/>модель из whitelist"]:::job
    A2 --> A3["Job UUID. Файлы на диск:<br/>data/jobs/uuid/inputs/"]:::job
    A3 --> A4["Сразу {job_id}. Анализ ещё идёт"]:::job
    A4 --> SSE["UI: GET .../stream SSE<br/>и поллинг статуса раз в 2 с"]:::ui
    A3 --> PIPE

    subgraph PIPE["Фиксированный конвейер — те же шаги для любого файла"]
        direction TB

        S1["1. Подготовка<br/>загрузить таблицы, превью 20 строк<br/>таблицы НЕ объединяются"]:::py
        S2["2. Связи таблиц — только в отчёт<br/>кандидаты join / union, сам join не выполняется"]:::py
        S3["3. Структура каждой таблицы<br/>kind столбца: numeric, categorical,<br/>datetime, boolean, identifier, textual"]:::py
        S4["4. Качество и корреляции<br/>балл 0–100, пропуски, константы<br/>Pearson / Cramér V / eta"]:::py
        S5["5. Инсайты и гипотезы Python<br/>выбросы, ядро/хвост категорий,<br/>подозрительные значения, профили групп<br/>гипотезы из расчётов, не из головы модели"]:::py
        S6["6. План и расчёт метрик pandas<br/>mean, median, nunique, date_range, …"]:::py

        S1 --> S2 --> S3 --> S4 --> S5 --> S6
        S6 --> S7a
        S6 --> S7b

        S7a["7a. Python рисует графики PNG<br/>бюджет graph_count делится между таблицами"]:::py
        S7b["7b. LLM один раз: 6–8 предложений<br/>комментария к готовым фактам<br/>нет Ollama → Python-бриф, пайплайн не падает"]:::llm

        S7a --> S8
        S7b --> S8

        S8["8. Итоговый отчёт TXT + DOCX без LLM<br/>характеристика, качество, инсайты,<br/>метрики, гипотезы, графики, ограничения"]:::py
        S9["9. store.complete → 100%"]:::job
        S8 --> S9
    end

    S9 --> R1["Вкладки UI уже наполнялись по ходу:<br/>preview, структура, связи, инсайты,<br/>метрики, анализ, гипотезы, графики, отчёт"]:::ui
    S9 --> R2["Скачать PNG / DOCX / TXT / XLSX<br/>из data/jobs/uuid/output/"]:::out
    SSE -.-> R1
```

Внутри шагов 3–6 для **каждой** таблицы вызывается один `analyze_one_table`: типы → качество → discovery → метрики. Пайплайн потом только склеивает тексты, рисует графики, зовёт LLM и собирает отчёт. Подробности этапов — [pipeline.md](pipeline.md).
