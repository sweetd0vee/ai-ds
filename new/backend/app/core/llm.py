...
async def chain_run(prompt_template: str, output_key: str, llm, **kwargs) -> str:
    prompt = PromptTemplate.from_template(prompt_template)
    chain = LLMChain(llm=llm, prompt=prompt, output_key=output_key)
    result = await asyncio.to_thread(chain.run, **kwargs)
    return result


async def chain_invoke(prompt_template: str, output_key: str, llm, inputs: dict | None = None, partial: dict | None = None) -> str:
    prompt = PromptTemplate.from_template(prompt_template)
    if partial:
