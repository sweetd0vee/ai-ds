...
Актуальный пайплайн — **Python-first** (метрики, гипотезы и графики на Python, LLM только комментирует факты). Вход в код: [docs/onboarding.md](docs/onboarding.md). Legacy Streamlit: `old/ins_temp3.py`, корневой `DOCUMENTATION.md`.
...
1. Загрузка таблиц и поиск связей (без join)
2. Структура столбцов (Python)
3. Качество и корреляции (Python)
4. Инсайты и гипотезы (Python)
5. План и расчёт метрик (Python)
6. Короткий комментарий LLM + графики PNG
7. Итоговый отчёт (Python, .txt + .docx)

Подробности: [docs/pipeline.md](docs/pipeline.md), [docs/onboarding.md](docs/onboarding.md)

## Docker

Production-стек в контейнерах: UI :8080, API :8020. См. [docs/docker.md](docs/docker.md) или `docker/README.md`.
...
| Долгий анализ | Нормально на больших таблицах и медленной модели; графики — Python, LLM только короткий текст |
